...
1. **Ускорение рефакторинга**: 
    ...
    который вызывает другие контроллеры (а я видел такое), но отрефакторить вам это запрещает вся команда, 
    потому что можно сломать базовую функциональность, 
...
1. [PHPUnit](https://github.com/sebastianbergmann/phpunit) 
    ...
    ```
Предоставляет assert, code coverage, mocks, группировка тестов, отчеты
...
**[7:50 - 5:30] Блок 3: Unit vs Интеграционные тесты**
...
**[7:50 - 10:50] Блок 4: Stubs vs Mocks** 
...
**[10:50 - 8:30] Блок 5: TDD и метрики**
